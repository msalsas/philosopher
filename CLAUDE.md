...
- `tts/` — **provider-selectable TTS** via `PHILOSOPHER_TTS_PROVIDER` (all env-driven, one WAV per sentence sent to the toy as a binary frame): **piper** (`engine.py`, default; the `piper` binary kept **resident** in `--json-input` mode so the voice model loads once), **kokoro** (`kokoro_engine.py`, local/offline neural via kokoro-onnx — no torch — the natural default we shipped, voice `em_santa`), **edge** (`edge_engine.py`, Microsoft online neural voices via edge-tts+ffmpeg — most natural but needs internet). All share a post pitch/formant shift (`PHILOSOPHER_TTS_PITCH`, <1 = deeper). The WS path synthesizes **per sentence for progressive playback**.
...
- **Servos move one at a time, sequentially.** The 512MB toy's MicroUSB power rail browns out if multiple servos start together — `hardware/servos.py` queues moves with delays. Don't parallelize servo motion. On the **real Banana Pi (H3)** the BCM pins (12/13/18) are meaningless and `OPi.GPIO`'s `PWM` is hardware-PWM-only (the M2 Zero header exposes none), so `servos.py` must be ported to `GPIO.SUNXI` + **bit-banged** PWM (see `scripts/servo_test.py`). 5V distribution to 3 servos + the amp needs a WAGO 221-415 (only 2 5V header pins). See `HARDWARE_RUNBOOK.md` §B.4.3.
- **TTS backend depends on the provider.** `piper` (default) is a **system binary** on PATH (not pip); `kokoro` needs the `.[tts-kokoro]` extra + system `espeak-ng` + its model (`download_models.py`); `edge` needs `.[tts-edge]` + system `ffmpeg` + internet. Any provider returning empty bytes (missing binary/model/network) degrades to text-only — the toy shows text but stays silent.
