...
- **Both reply paths share assembly + fallbacks.** HTTP `node_think` and WS `_stream_response` both call `build_messages(state)` (conversation only; the new-face intro / name-ask guidance is in the system prompt via `build_prompt`, not bracketed user turns) so they can't drift. Fallbacks are **localized** via the personality YAML (`fallbacks:` section → `personality.fallback(key)`), not hardcoded English.
...
- **Noise is ignored silently; unclear *speech* asks to repeat.** When STT yields no usable transcript, `process_speech` splits two cases: **background noise** (empty text, *or* whisper's averaged `no_speech_prob` ≥ `PHILOSOPHER_STT_NO_SPEECH_THRESHOLD`, default 0.6) → a silent `{type:"idle"}` (mic gate released, nothing spoken), so a slamming door doesn't make the toy nag; **unclear speech** (a transcript exists but `low_confidence`, i.e. `avg_logprob < CONFIDENCE_THRESHOLD`) → the spoken `not_understood` fallback. The wake gate follows the same split (noise always silent; unclear speech asks to repeat only while awake).
