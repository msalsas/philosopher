...
```
External LLM  ──HTTP──▶  philosopher-server (RPi4, the brain)  ◀──WebSocket──▶  philosopher-toy (Pi Zero WH, I/O only)
...
- **Servos move one at a time, sequentially.** The 512MB toy's MicroUSB power rail browns out if multiple servos start together — `hardware/servos.py` queues moves with delays. Don't parallelize servo motion. On the **Pi Zero WH** the BCM pins (12/13/18) are the correct ones (RPi.GPIO); servos are not wired yet. 5V distribution to 3 servos + the amp still needs a solid supply (the MicroUSB rail browns out under load). See `HARDWARE_RUNBOOK.md`.
